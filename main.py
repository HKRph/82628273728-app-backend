...
from sqlalchemy import create_engine, Column, Integer, BigInteger, String, Float, ForeignKey, Text, Date, Boolean, or_
...
class TaskSubmission(Base):
    ...
    is_read = Column(Boolean, default=False) # NEW: For notification system
    ...
    try:
        ...
        if user_db.status == 'restricted' and user_db.status_until and user_db.status_until > date.today():
            ...
            raise HTTPException(status_code=403, detail=f"You are restricted until {user_db.status_until.strftime('%b %d')}.")
             ...
             # Lift restriction if date has passed
...
async def mark_notifications_as_read(request: Request):
    """Marks all notifications (task submissions and withdrawals) for a user as read."""
    try:
        data = await request.json(); user_id = data.get('user_id')
        if not user_id:
            raise HTTPException(status_code=400, detail="user_id not provided")

        # Mark all unread task submissions as read
        db_session.query(TaskSubmission).filter(
            TaskSubmission.user_id == user_id,
            TaskSubmission.is_read == False
        ).update({"is_read": True})

        # Mark all unread withdrawals as read
        db_session.query(Withdrawal).filter(
            Withdrawal.user_id == user_id,
            Withdrawal.is_read == False
        ).update({"is_read": True})
...
async def submit_task_proof(request: Request):
    try:
        ...
        if not task: # Should ideally not happen if task existed in frontend
        ...
        try:
            ...
            # Update user's username in DB if available and changed
            if user_tg_obj.username and user_db.username != user_tg_obj.username:
                user_db.username = user_tg_obj.username
                db_session.commit() # Commit this small update separately
        except Exception as e:
            logger.warning(f"Could not fetch live Telegram user object for {user_id}: {e}")

        username_str = f" (@{user_db.username})" if user_db.username else "" # Use DB stored username
    ...
    try:
        ...
        if user_db.last_login_date is None or user_db.last_login_date < date.today():
            if user_db.daily_claim_invites >= DAILY_BONUS_INVITE_REQ:
                ...
                user_db.daily_claim_invites = 0 # Reset invites after claiming
                db_session.commit()
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_db:
        user_db = User(id=user.id, first_name=user.first_name, username=user.username) # Store username on first interaction
    ...
    # --- UPDATED START COMMAND CAPTION AND BUTTONS ---
    if user_db.status == 'banned': 
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("📢 Join Channel", url=XEWEE_NEWS_CHANNEL_URL)] # Link to your news channel
        ]
    # --- END UPDATED START COMMAND CAPTION AND BUTTONS ---
...
async def admin_main_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check if the message content or reply_markup is different before editing
    if query.message.text != "👑 **Xewee Admin Dashboard**" or query.message.reply_markup != InlineKeyboardMarkup(keyboard):
        await query.message.edit_text("👑 **Xewee Admin Dashboard**", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
    else:
        logger.info("Admin dashboard message not modified, skipping edit.")
...
async def admin_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.message.text != stats_message or query.message.reply_markup != InlineKeyboardMarkup(keyboard):
        await query.message.edit_text(stats_message, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
    else:
        logger.info("Admin stats message not modified, skipping edit.")
...
async def manage_tasks(update: Update, context: ContextTypes.DEFAULT_TYPE): 
    ...
    if query.message.text != "Manage tasks:" or query.message.reply_markup != InlineKeyboardMarkup(keyboard):
        await query.message.edit_text("Manage tasks:", reply_markup=InlineKeyboardMarkup(keyboard))
    else:
        logger.info("Manage tasks message not modified, skipping edit.")
...
async def remove_task_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    tasks = db_session.query(Task).all()
    if not tasks: 
        if query.message.text != "No tasks to remove." or query.message.reply_markup:
    ...
    if query.message.reply_markup != InlineKeyboardMarkup(keyboard):
        await query.message.edit_text("Select a task to remove:", reply_markup=InlineKeyboardMarkup(keyboard));
    else:
        logger.info("Remove task list message not modified, skipping edit.")
...
async def manage_codes(update: Update, context: ContextTypes.DEFAULT_TYPE): 
    ...
    keyboard = [[InlineKeyboardButton("➕ Add New Code", callback_data="add_code_start")], 
                ...
                [InlineKeyboardButton("⬅️ Back to Admin Menu", callback_data="admin_main_menu")]]
    if query.message.text != "Manage redeem codes:" or query.message.reply_markup != InlineKeyboardMarkup(keyboard):
        await query.message.edit_text("Manage redeem codes:", reply_markup=InlineKeyboardMarkup(keyboard))
    else:
        logger.info("Manage codes message not modified, skipping edit.")
...
async def remove_code_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not codes: 
        if query.message.text != "No codes to remove." or query.message.reply_markup:
    ...
    if query.message.reply_markup != InlineKeyboardMarkup(keyboard):
        ...
        logger.info("Remove code list message not modified, skipping edit.")
...
async def user_mgt_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[InlineKeyboardButton("Ban 🔨", callback_data="user_mgt_ban")], 
                ...
                [InlineKeyboardButton("⬅️ Back to Admin Menu", callback_data="admin_main_menu")]]
    if query.message.text != "User Management:" or query.message.reply_markup != InlineKeyboardMarkup(keyboard):
        await query.message.edit_text("User Management:", reply_markup=InlineKeyboardMarkup(keyboard))
    else:
        logger.info("User management message not modified, skipping edit.")
...
async def user_search_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # If not an ID, try searching by username from DB (if stored) or Telegram API
        username_query = search_query
        if username_query.startswith('@'):
            username_query = username_query[1:]
        
        # First, try to find in our DB by stored username
        user_db = db_session.query(User).filter(User.username == username_query).first()
        
        if not user_db: # If not found in DB by username, try Telegram API
            try:
                user_tg_obj = await ptb_app.bot.get_chat(f"@{username_query}")
                # If found by Telegram API, try to find in our DB by ID
                user_db = db_session.query(User).filter(User.id == user_tg_obj.id).first()
                if user_db: # Update username if found and different
                    if user_db.username != user_tg_obj.username:
                        user_db.username = user_tg_obj.username
                        db_session.commit()
                else: # If found on Telegram but not in our DB, create new user (partial)
                    user_db = User(id=user_tg_obj.id, first_name=user_tg_obj.first_name, username=user_tg_obj.username)
                    db_session.add(user_db)
                    db_session.commit()
                    logger.info(f"User {user_db.id} (@{user_db.username}) added to DB during admin search.")
            except Exception as e:
                logger.warning(f"Could not fetch Telegram user by username @{username_query}: {e}")
                user_db = None # Ensure user_db is None if API lookup fails


    if user_db:
        username_display = f"@{user_db.username}" if user_db.username else "N/A"
        first_name_display = user_db.first_name or "Unknown" # Use DB stored first_name
        ...
        await update.message.reply_text(f"User '{search_query}' not found by ID or username.")
...
async def review_submissions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if message_to_edit and message_to_edit.photo: # If the current message already has a photo
            # Only edit if caption changed to avoid BadRequest
            if (message_to_edit.caption is None and caption_text) or \
               (message_to_edit.caption is not None and message_to_edit.caption != caption_text):
        ...
        if message_to_edit:
            if (message_to_edit.text is None and new_text_content) or \
               (message_to_edit.text is not None and message_to_edit.text != new_text_content) or \
               (message_to_edit.reply_markup != InlineKeyboardMarkup(keyboard)): # Check if reply_markup also changed
...
async def approve_submission(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_db or user_db.status != 'active':
        ...
        submission.is_read = False # Mark as unread for user to see rejection
        db_session.commit()
        current_caption = query.message.caption if query.message.caption else ""
    ...
    submission.is_read = False # Mark as unread for user to see approval
    ...
    try:
        ...
        if task.id not in completed_task_ids_list:
            ...
            # --- REFERRAL COMMISSION LOGIC (Tiered commission can be implemented here) ---
        ...
        await review_submissions(update, context) # Try to continue
...
async def get_submission_rejection_reason(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if admin_message_to_edit:
            ...
            # Check if message is already modified to avoid BadRequest
            if (admin_message_to_edit.caption is None and new_caption) or \
               (admin_message_to_edit.caption is not None and admin_message_to_edit.caption != new_caption):
...
async def approve_withdrawal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        withdrawal.is_read = False # Mark as unread for user to see approval
        db_session.commit(); 
...
async def get_withdrawal_rejection_reason(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        withdrawal.is_read = False # Mark as unread for user to see rejection
        db_session.commit()
